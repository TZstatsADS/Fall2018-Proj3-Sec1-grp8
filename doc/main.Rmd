...
model <- neuralnet(label1+label2+label3+label4~feature1+feature2+feature3+feature4+feature5+feature6+feature7+feature8,data=dataset,hidden=1,linear.output = T)
plot(model)

df1 <- data.frame(matrix(dat_train$feature[1:100000,,],ncol=8))
dim(df1)
df2 <- data.frame(matrix(dat_train$label[1:100000,,],ncol=4))
colnames(df1) <- paste0("feature", 1:8) 
colnames(df2) <- paste0("label", 1:4)
...
for (i in 1:12){
    ...
    df1 <- data.frame(matrix(feat_train,ncol=8))
    df2 <- data.frame(matrix(label_train,ncol=4))
    colnames(df1) <- paste0("feature", 1:8) 
    colnames(df2) <- paste0("label", 1:4)
    dataset <- cbind(df1, df2)
    model <- neuralnet(label1+label2+label3+label4~feature1+feature2+feature3+feature4+feature5+feature6+feature7+feature8,data=dataset,hidden=1,linear.output = T)
    ...
    time <- end -start
    modelList[[i]] <- list(fit=model,times=time)
}
```
...
colnames(df2) <- paste0("label", 1:4)
dataset <- cbind(df1, df2)
model <- neuralnet(label1+label2+label3+label4~feature1+feature2+feature3+feature4+feature5+feature6+feature7+feature8,data=dataset,hidden=2,linear.output = T)
end <- Sys.time()
time <- end -start
print(time)
plot(model)
